...
const categoria = async (palavra) => {
  const categoria = await fetch(`https://api.mercadolibre.com/sites/MLB/search?category=${palavra}`)
  const categoriaJson = await categoria.json();
  return categoriaJson
}

const termo = async (b) => {
  const termo = await fetch(`https://api.mercadolibre.com/sites/MLB/search?q=${b}`)
  const termoJson = await termo.json();
  return termoJson
}

const categoryTermo = async (a,b) => {
  const api = await fetch(`https://api.mercadolibre.com/sites/MLB/search?category=${a}&q=${b}`)
  const fetchTudo = await api.json();
  return fetchTudo
}

const detalhes = async (a) => {
  const api = await fetch(`https://api.mercadolibre.com/items/${a}`)
